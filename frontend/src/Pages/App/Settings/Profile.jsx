...
export default function Profile() {
  ...
  let disabled = email === user?.email && fullname === user?.fullname
  

  const Save = (e) => {
    e.preventDefault();

    axios.put(`${BackendURL}/users/${user?.id}`, {email, fullname})
      .then(res => {
        ...
        <main className='bg-gray-100 w-full px-10 min-h-screen pb-20'>
          ...
          <div className="px-8 ">
            ...
            <div className="bg-white shadow pb-10 mt-8 rounded-md">
              <h1 className="px-10 py-4 font-medium text-lg text-gray-600"> {lang.informations} </h1>
              <hr className="text-gray-300"></hr>

              <form onSubmit={Save} className="w-2/5 min-h-90 px-10 mt-6">
                <div className="mb-4 flex flex-col">
                  <label className="text-gray-500 font-medium mb-2"> {lang.fullname} </label>
                  <input type="text" onChange={(e)=> setFullname(e.target.value)} value={fullname} className="border rounded-md border-gray-500 py-2 px-4" />
                </div>
                <div className="mb-4 flex flex-col">
                  <label className="text-gray-500 font-medium mb-2"> {lang.email} </label>
                  <input type="text" onChange={(e)=> setEmail(e.target.value)} value={email} className="border rounded-md border-gray-500 py-2 px-4" />
                </div>
